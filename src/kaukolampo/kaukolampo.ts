import {AdjustedYearTotal, ContractPricing, MonthlyPrice, PowerUsage, YearMonth, YearTotal} from "./kaukolampoTypes";
import {isDefined} from "../../../ki-frame/src/util/typeUtils";
import {b, div, h2, h3, li, p, style, table, tbody, td, th, thead, tr, ul} from "../../../ki-frame/src/domBuilder";
...
export function pricingSummary(contract: ContractPricing, years: number[], monthlyPricing: Record<number, MonthlyPrice>) {
  const months = range(1, 12);
  return div(h2(`${contract.companyName} ${contract.contractTypeName} hinnasto ${years[0]}-${years[years.length - 1]}`),
    table(
      styles({width: "auto"}),
      thead(
        tr(
        ...
        tr(
          years.map(y => [th("kk €"), th("€/MWh")])
        )
      ),
      tbody(
        months.map(month => tr(td(month), years.map(year => {
...
const showIncrease = (inc?: number) => (!inc || inc === 0) ? {} : inc > 0 ? {backgroundColor: 'lightpink'} : {backgroundColor: 'lightgreen'}

export function billSummary(address: string, years: number[], monthlyPricing: Record<number, MonthlyPrice>, powerUsage: PowerUsage) {
  const borderLeft = {styles: {borderLeft: "2px solid #6b7280"}};
  const months = range(1, 12);
  const totalsByYear: Record<number, YearTotal> = {}
  const billRows = months.map(month => tr(td(month), years.map(year => {
  ...
  return {
    bills: div(h2(`${address} laskut ${years[0]}-${years[years.length - 1]}`),
      table(
        styles({width: "auto"}),
        thead(
          tr(
...
function calculateNewPricingWithMax150PriceIncrease(totalWithLastYearLevel: Decimal, monthlyFee: Decimal, mwPrice: Decimal, totalEnergy: Decimal): AdjustedYearTotal {
...
function compareYears(comparedYears: number[], adjustedPricing: Record<number, AdjustedYearTotal>, totalsByYear: Record<number, YearTotal>, excessBillingAll: Record<number, Decimal>, years: number[]) {
      ...
      return div(
        ...
        p(
          ...
          ul(
            ...
            li(`Vuoden ${y} energiakulutus ${printPower(totals.totalEnergy)} vuoden (${y - 1}) kuukausimaksulla ja energian hinnalla: `,
              `${printPower(totals.totalEnergy)} * ${printMoney(prevAvgMwPrice)} + ${totals.monthCount} * ${printMoney(prevAvgMonthlyFee)} = `, b(printMoney(totalWithPrevYearLevel)))
          )
        ),
        p(`Korotus ${y} vs ${prevYear} tasolla`,
          ul(
            li(`${y} yhteensä ${printMoney(totalWithThisYearLevel)}, ${prevYear} tasolla ${printMoney(totalWithPrevYearLevel)}`),
...
function excessBillingPaybackInterest(excessYears: number[], adjustedPricing: Record<number, AdjustedYearTotal>, monthlyPricing: Record<number, MonthlyPrice>, powerUsage: PowerUsage) {
  ...
  return table(
    ...
    tbody(
      excessYears.map(year => range(1, 12).map(month => {
        const index = ymToIndex({year, month})
        const power = powerUsage.numbers[index]
        ...
        const interestMultiplier = calculateViivastyskorkoMultiplier(toDateISO(`${year}-${month}-1`), toDateISO(`${year}-${month}-1`), true)
...
export function evaluatePriceIncreases(address: string, years: number[], totalsByYear: Record<number, YearTotal>, monthlyPricing: Record<number, MonthlyPrice>, powerUsage: PowerUsage) {
  ...
  return div(
    ...
    Object.keys(excessBillingAll).length > 0 && [
      ...
      p("Viivästyskorko lasketaan korjattujen kuukausien laskujen maksupäivästä. Käytännössä siloin on laskutettu liikaa, jolloin rahaa on kerätty perusteettomasti"),
