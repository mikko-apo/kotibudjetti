...
(() => {
  ...
  function calculateSellSummary(params) {
    ...
    const selectedHmo20DeductionTotal = sumDecimals(
      usedLots.filter((lot) => lot.selectedMethod === "hmo" && lot.hankintamenoOlettaRate.eq(shortOwnershipRate)).map((lot) => lot.hankintamenoOlettaDeduction)
    );
    const selectedHmo40DeductionTotal = sumDecimals(
      usedLots.filter((lot) => lot.selectedMethod === "hmo" && lot.hankintamenoOlettaRate.eq(longOwnershipRate)).map((lot) => lot.hankintamenoOlettaDeduction)
    );
  ...
  function buildIpoSummaryFromInputs(ipoInput, ipoSellInput, ipoDate, totalSubscribedShares, totalSubscribedCost, warnings, localization) {
  ...
  function calculateVestingSummary(lots, ipoDate) {
    ...
    const sellableLots = ipoEligibleLots.filter(
      (lot) => lot.shareCount.gt(0) && (!lot.vestingEndsOnValue || !!(ipoDate && ipoDate.getTime() >= lot.vestingEndsOnValue.getTime()))
    );
  ...
  function calculateIpoSellSummary(sellableLots, sellShareCalculator, ipoSellAmount, ipoSellPricePerShare, ipoSellCostPerShare, otherAnnualCapitalGainsOrLosses, vestingSummary, ipoSummary, sellRules, errors, warnings, localization) {
  ...
  function parseOsakkeetCalculatorInputs(form2, localization) {
    ...
    const sells = form2.sells.map((input2) => {
      ...
      const shareCount = parseDecimalInput(input2.shareCount, sellFieldLabel(input2.id, "m\xE4\xE4r\xE4"), errors, validation, {
        validate: (value) => value.gt(0)
      });
    ...
    for (const event of sortEvents(events2)) {
      ...
      if (event.kind === "capitalRepaymentOrDividend") {
        ...
        for (const subscriptionId of lotOrder) {
          ...
          const beforeShareCount = lot.shareCount;
          if (beforeShareCount.lte(0)) continue;
          const isPastCutoff = options.forceTreatCapitalReturnsAsDividends || options.capitalReturnCutoffTimestampMs != null && !Number.isNaN(options.capitalReturnCutoffTimestampMs) && currentEventTimestampMs >= options.capitalReturnCutoffTimestampMs;
          ...
          const capitalRepaymentPerShare = !isPastCutoff && isEligibleByAge && lot.shareAcquisitionCost.gt(0) ? decimal_default.min(event.amountPerShare, remainingPerShare) : zero4;
          ...
          if (appliedShareAcquisitionCost.gt(0)) {
            lot.shareAcquisitionCost = decimal_default.max(lot.shareAcquisitionCost.minus(appliedShareAcquisitionCost), zero4);
          }
          recordLogEntry(internalState, subscriptionId, {
      ...
      if (event.kind === "sell") {
        ...
        for (const subscriptionId of lotOrder) {
          ...
          const beforeShareAcquisitionCost = lot.shareAcquisitionCost;
          const beforeBaseShareAcquisitionCost = lot.baseShareAcquisitionCost;
          const soldShareAcquisitionCost = beforeShareCount.gt(0) ? beforeShareAcquisitionCost.mul(soldShareCount).div(beforeShareCount) : zero4;
          const soldBaseShareAcquisitionCost = beforeShareCount.gt(0) ? beforeBaseShareAcquisitionCost.mul(soldShareCount).div(beforeShareCount) : zero4;
          lot.shareCount = decimal_default.max(lot.shareCount.minus(soldShareCount), zero4);
          lot.shareAcquisitionCost = decimal_default.max(lot.shareAcquisitionCost.minus(soldShareAcquisitionCost), zero4);
  ...
  function collectUnsupportedYearWarnings(cashDistributions, ipoDate, ipoSellAmount, warnings, localization, useYearlyRules) {
    ...
    ;
    [...unsupportedYears].sort((a2, b2) => a2 - b2).forEach((year) => warnings.push(localization.calculator.warnings.unsupportedYearRange(year)));
  }
  ...
  function getLatestLotStateOrZero(shareCalculator, subscriptionId) {
    ...
    const baseShareAcquisitionCost = result.log.filter(
      ...
      (subscriptionEntry == null ? void 0 : subscriptionEntry.kind) === "subscription" ? subscriptionEntry.shareAcquisitionCost : zero5
    );
    return {
  ...
  function deriveWorkingLot(lot, shareCalculator, options = {}) {
    ...
    const state = atDate ? getLotStateAtOrZero(shareCalculator, lot.id, atDate, inclusive) : getLatestLotStateOrZero(shareCalculator, lot.id);
    return {
      ...lot,
      ...state,
      ...
      ...includeAcquisitionCostAdjustments ? { acquisitionCostAdjustments: buildAcquisitionCostAdjustments(lot, shareCalculator) } : {}
    };
  }
  ...
  function buildFinalLotsFromShareCalculator(baseLots, shareCalculator) {
    return baseLots.map(
      (lot) => deriveWorkingLot(lot, shareCalculator, {
  ...
  function applyCashDistributions(lots, cashDistributions, shareCalculator, ipoDate, forceTreatAllDistributionsAsListedDividends, mathematicalShareValuesByYear, rules, warnings, localization, useYearlyRules) {
    const capitalDividendUsedByYear = /* @__PURE__ */ new Map();
    const grossDividendUsedByYear = /* @__PURE__ */ new Map();
    const summaries = [];
    const lotsById = new Map(lots.map((lot) => [lot.id, lot]));
    const sameDayLotStates = /* @__PURE__ */ new Map();
    let activeDistributionDayKey;
    const getDistributionDayKey = (entry) => entry.dateValue ? String(entry.dateValue.getTime()) : `raw:${entry.date}`;
    for (const entry of [...cashDistributions].sort((a2, b2) => compareDateStrings(a2.date, b2.date))) {
      ...
      const effectiveType = treatedAsListedDividend ? "dividend" : entry.type;
      const lotStates = eligibleLots.map((lot) => {
        const before = sameDayLotStates.get(lot.id) || (cashDistributionDate ? getLotStateAtOrZero(shareCalculator, lot.id, cashDistributionDate, false) : getLatestLotStateOrZero(shareCalculator, lot.id));
        return { lot, before };
      });
      ...
      const allocations = lotStates.map(({ lot, before }) => {
        ...
        const remainingPerShare = shares.gt(0) ? before.shareAcquisitionCost.div(shares) : zero5;
        const capitalRepaymentPerShare = effectiveType === "capital_return" && eligibleByAge && before.shareAcquisitionCost.gt(0) ? decimal_default.min(amountPerShare, remainingPerShare) : zero5;
        const capitalRepayment = capitalRepaymentPerShare.mul(shares);
        const after = {
          ...before,
          shareAcquisitionCost: effectiveType === "capital_return" ? decimal_default.max(before.shareAcquisitionCost.minus(capitalRepayment), zero5) : before.shareAcquisitionCost
        };
        const dividend = decimal_default.max(gross.minus(capitalRepayment), zero5);
        const dividendReason = effectiveType !== "capital_return" ? entry.type === "capital_return" ? "listed_dividend" : void 0 : !eligibleByAge ? "too_old" : dividend.gt(0) ? before.shareAcquisitionCost.lte(0) ? "no_remaining_cost" : "remaining_cost_limit" : void 0;
        const eligibleCapitalRepayment = effectiveType === "capital_return" && eligibleByAge && capitalRepayment.gt(0);
      ...
      const capitalDividendGross = shareholderMathematicalValue.gt(0) ? decimal_default.min(dividendTotal, eightPercentYieldLimit) : zero5;
      const earnedDividendGross = shareholderMathematicalValue.gt(0) ? decimal_default.max(dividendTotal.minus(capitalDividendGross), zero5) : zero5;
      ...
      const taxableCapitalIncome = treatedAsListedDividend ? dividendTotal.mul(distributionRules.listedDividend.taxableCapitalIncomeRate) : lowCapitalPart.mul(distributionRules.unlistedDividend.lowCapitalDividendTaxableRate).add(highCapitalPart.mul(distributionRules.unlistedDividend.highCapitalDividendTaxableRate));
      const taxFreeCapitalIncomePortion = treatedAsListedDividend ? dividendTotal.mul(distributionRules.listedDividend.taxFreeCapitalIncomeRate) : lowCapitalPart.mul(distributionRules.unlistedDividend.lowCapitalDividendTaxFreeRate).add(highCapitalPart.mul(distributionRules.unlistedDividend.highCapitalDividendTaxFreeRate));
      const taxableEarnedDividend = treatedAsListedDividend ? zero5 : earnedDividendGross.mul(distributionRules.unlistedDividend.earnedDividendTaxableRate);
      const taxFreeEarnedDividend = treatedAsListedDividend ? zero5 : earnedDividendGross.mul(distributionRules.unlistedDividend.earnedDividendTaxFreeRate);
      ...
      const withholdingToTaxOffice = treatedAsListedDividend ? dividendTotal.mul(distributionRules.listedDividend.withholdingRate) : lowWithholdingPart.mul(distributionRules.unlistedDividend.lowWithholdingRate).add(highWithholdingPart.mul(distributionRules.unlistedDividend.highWithholdingRate));
  ...
  function buildTaxReturnYearSummaries(cashDistributions, sales, ipoDate, mathematicalShareValuesByYear, shareCalculator) {
  ...
  function buildTaxReturnSaleSummaries(sells, baseLots, shareCalculator, ipoDate, ipoSell, effectiveRules, useYearlyRules) {
    const historicalSales = sells.flatMap((sell) => {
      ...
      const sellableLots = baseLots.map((lot) => deriveWorkingLot(lot, shareCalculator, { atDate: sellDate, inclusive: false })).filter((lot) => lot.shareCount.gt(0));
  ...
  function calculateOsakkeet(form2, localization, rules) {
    ...
    const warnings = [];
    const forceTreatAllDistributionsAsListedDividends = parsed.company.listingStatus === "listed" && !parsed.company.becameListedDate;
    ...
    const ipoLots = baseLots.map(
      (lot) => deriveWorkingLot(lot, baseShareCalculator, { atDate: ipoDate, inclusive: true })
    );
    ...
    const ipoRelevantSells = ipoDate ? sortedSells.filter((sell) => compareDateStrings(sell.date, parsed.company.becameListedDateText) <= 0) : sortedSells;
    const { shareCalculator: sellShareCalculator, errors: sellShareCalculatorErrors } = createShareCalculator(
      {
        ...
        sells: ipoDate && ipoSellAmount.gt(0) ? [
          ...ipoRelevantSells,
          {
            kind: "sell",
            id: "ipo-sell",
    ...
    const sellRules = resolveYearlyTaxRules((_a2 = ipo.ipoDate) == null ? void 0 : _a2.getUTCFullYear(), effectiveRules, useYearlyRules);
  ...
  function mapStatePathsToInputs(state, bindings) {
    const handles = bindings.map(({ path, node, options }) => {
      ...
      const write = (options == null ? void 0 : options.write) || ((currentNode, value) => writeDefaultNodeValue(currentNode, value));
      const writeStateToNode = (stateValue) => {
        write(node, getStateValueAtPath(stateValue, path));
      };
      writeStateToNode(state.get());
  ...
  function mapStateToDomChildren(state, root, optionsOrRender) {
    ...
    const sync = (stateValue) => {
      ...
      const activeElement = typeof document !== "undefined" && document.activeElement instanceof Node ? document.activeElement : null;
  ...
  var FI = {
    ...
    company: {
      ...
      help: "Valitse onko yhti\xF6 t\xE4ll\xE4 hetkell\xE4 listaamaton vai listattu. Valinnainen listautumisp\xE4iv\xE4 toimii t\xE4ss\xE4 laskurissa IPO-p\xE4iv\xE4n\xE4 varojenjaon ja IPO-myynnin rajap\xE4iv\xE4n\xE4. Jos yhti\xF6 on merkitty listatuksi eik\xE4 listautumisp\xE4iv\xE4\xE4 anneta, kaikki varojenjaot k\xE4sitell\xE4\xE4n t\xE4ss\xE4 n\xE4kym\xE4ss\xE4 osinkoina.",
      fields: {
    ...
    mainSections: {
      ...
      groups: {
        ...
        distributionsAndCorporateActions: {
          title: "1. Yrityksen tiedot: Varojenjako, jakautuminen ja splitit. Maksut osakkeiden perusteella ja yhteenveto verotuksen n\xE4k\xF6kulmasta",
          summary: "Sis\xE4lt\xE4\xE4: Yrityksen tila ja listautumisp\xE4iv\xE4, Osingot ja p\xE4\xE4omanpalautukset, Yrityksen jakautuminen hankintamenon mukaan, Osakesplitit."
        },
        taxReturns: {
    ...
    subscriptions: {
      ...
      history: {
        ...
        details: {
          ...
          dividend: (amountPerShare, shares, total) => `Osinko ${amountPerShare} / osake x ${shares} osaketta = ${total}.`,
          dividendOnlyWithReason: (dividendPerShare, dividendTotal, reason) => `Osinkona ${dividendPerShare} / osake = ${dividendTotal} (${reason}).`,
    ...
    cashDistributions: {
      title: "Yrityksen osingot ja p\xE4\xE4omanpalautukset. Maksut osakkeenomistajalle ja verottajalle ja p\xE4\xE4omanpalautus/osinko erottelu",
      ...
      headerGroups: {
        distribution: "Osinko tai p\xE4\xE4omanpalautus:",
      ...
      actions: {
        ...
        showLotEvents: "Er\xE4kohtaiset tapahtumat",
        hideLotEvents: "Piilota er\xE4kohtaiset tapahtumat"
      },
      sections: {
        lotEvents: "Merkint\xE4eritt\xE4in"
      },
      types: {
    ...
    demergers: {
      ...
      fields: {
        ...
        exampleEffect: "Esimerkki",
        exampleEffectValue: (ratio, oldCompany, newCompany) => `10,00 \u20AC -> vanha yhti\xF6 ${oldCompany}, uusi yhti\xF6 ${newCompany}`
      },
      actions: {
    ...
    summary: {
      ...
      ipoSell: {
        ...
        cashReserve: {
          ...
          otherAnnualCapitalGainsOrLossesHelp: "Sy\xF6t\xE4 kentt\xE4\xE4n muut mahdolliset luovutusvoitot ja tappiot ja niiden yhteisarvo",
          ...
          remainingSharesUnvestedLine: (shares, ipoValue, originalAcquisitionCost) => `Ansaintajakson piiriss\xE4: ${shares} osaketta, arvo IPO-hinnalla ${ipoValue}, alkuper\xE4inen hankintameno ${originalAcquisitionCost}`,
          reserveForTaxes: "Veroihin varattava",
          taxEffectFromOtherAnnualCapital: "Muiden luovutusvoittojen tai -tappioiden vaikutus veron m\xE4\xE4r\xE4\xE4n",
          ...
          remainingSharesHelp: (shareValue, totalValue) => `Arvo ${totalValue} on laskettu IPO-hinnalla ${shareValue} / osake.`,
        ...
        saleResultComparison: {
          ...
          value: (original, gain, percent) => `Alkuper\xE4inen hankintameno ${original}, nettotulos ${gain} (${percent}).`,
          help: (original, kept, gain, percent) => `Myynniss\xE4 k\xE4ytettyjen merkint\xE4erien alkuper\xE4inen hankintameno on ${original}. Tilille voi j\xE4tt\xE4\xE4 ${kept}, joten nettotulos k\xE4ytettyihin merkint\xE4eriin n\xE4hden on ${gain} (${percent}).`
        },
        ipoCostEffects: {
    ...
    calculator: {
      ...
      warnings: {
        ...
        unsupportedYearRange: (year) => `Vuositason vero-, osinko- ja p\xE4\xE4omanpalautuslaskenta on tuettu verovuosille 2016 ja sit\xE4 uudemmille. Sy\xF6tteiss\xE4 on vuosi ${year}.`,
        ipoSellAmountExceedsEstimatedSecondary: "Myyntim\xE4\xE4r\xE4 ylitt\xE4\xE4 arvioidun secondary-myyntim\xE4\xE4r\xE4n koko yhti\xF6n tasolla.",
        vestingBlockedWithoutIpoDate: "Listautumisp\xE4iv\xE4 puuttuu, joten ansaintajakson rajoittamia merkint\xE4eri\xE4 ei voitu ottaa mukaan myyntiin."
      },
      errors: {
        ipoSellPricePerShareRequired: "IPO-hinta / osake pit\xE4\xE4 sy\xF6tt\xE4\xE4 ennen kuin IPO-myynnin arvot voidaan laskea.",
        ipoSellAmountExceedsSellable: (shares) => `Myyt\xE4vien osakkeiden m\xE4\xE4r\xE4 ylitt\xE4\xE4 listautumisp\xE4iv\xE4n\xE4 myyt\xE4viss\xE4 olevien osakkeiden m\xE4\xE4r\xE4n (${shares}).`
      }
    }
  };
  var EN = {
    ...
    mainSections: {
      ...
      groups: {
        ...
        distributionsAndCorporateActions: {
          title: "1. Company details: distributions, demergers, and splits. Share-based payments and a tax-focused summary",
          summary: "Includes: Company status and became-listed date, Dividends and capital repayments, Company demerger by acquisition-cost allocation, Share splits."
        },
        taxReturns: {
          title: "3. Tax returns",
    ...
    subscriptions: {
      ...
      history: {
        ...
        details: {
          ...
          dividend: (amountPerShare, shares, total) => `Dividend ${amountPerShare} / share x ${shares} shares = ${total}.`,
          dividendOnlyWithReason: (dividendPerShare, dividendTotal, reason) => `As dividend ${dividendPerShare} / share = ${dividendTotal} (${reason}).`,
    ...
    cashDistributions: {
      title: "Company dividends and capital repayments. Payments to the shareholder and tax authority, with a capital-repayment/dividend breakdown",
    ...
    demergers: {
      ...
      fields: {
        ...
        exampleEffect: "Example",
        exampleEffectValue: (ratio, oldCompany, newCompany) => `10.00 EUR -> old company ${oldCompany}, new company ${newCompany}`
      },
      actions: {
        add: "Add demerger"
      }
    ...
    summary: {
      ...
      ipoSell: {
        ...
        cashReserve: {
          ...
          otherAnnualCapitalGainsOrLossesHelp: "Enter any other possible capital gains and losses and their combined amount in this field",
          ...
          remainingSharesUnvestedLine: (shares, ipoValue, originalAcquisitionCost) => `Unvested now: ${shares} shares, value at IPO price ${ipoValue}, original acquisition cost ${originalAcquisitionCost}`,
          ...
          remainingSharesHelp: (shareValue, totalValue) => `Value ${totalValue} is calculated using the IPO price ${shareValue} / share.`,
          ...
          taxEffectFromOtherAnnualCapitalHelp: (other, reduction, increase) => `Entered change ${other}. A negative value reduces the tax estimate by ${reduction}. A positive value increases the tax estimate by ${increase}. Selling shares that are down can reduce tax, but an immediate buyback should not be done solely for tax reasons without professional advice.`,
        ...
        saleResultComparison: {
          ...
          help: (original, kept, gain, percent) => `The original acquisition cost of the subscription lots used in the sale is ${original}. You can keep ${kept}, so the net result against the sold subscription lots is ${gain} (${percent}).`
        },
        ipoCostEffects: {
    ...
    calculator: {
      ...
      warnings: {
        ...
        unsupportedYearRange: (year) => `Year-level tax, dividend, and capital-repayment calculations are supported for tax years 2016 and later. The input contains year ${year}.`,
        ipoSellAmountExceedsEstimatedSecondary: "Sell amount exceeds the estimated secondary sell amount at whole-company level.",
        vestingBlockedWithoutIpoDate: "The became-listed date is missing, so vesting-restricted subscription lots were excluded from the sale."
      },
      errors: {
        ipoSellPricePerShareRequired: "IPO price / share must be entered before the IPO sell values can be calculated.",
        ipoSellAmountExceedsSellable: (shares) => `The number of shares to sell exceeds the shares sellable on the became-listed date (${shares}).`
      }
    }
  };
  ...
  function numberInput(inputStyle, value, onInput = () => {
  }, numeric = true) {
    return inputs.text(
      {
        value,
  ...
  var zero6 = new decimal_default(0);
  function createCompanySection(dataState, pageReadState, localizedTextNodes) {
    ...
    const listingStatusSelect = createOptionBoundSelect(
  ...
  function createSellsSection(dataState, pageReadState, localizedTextNodes, commonTextNodes) {
    ...
    const rowsState = pageReadState.map(
      ({ osakkeetCalculation, texts }) => withRowActionLabels(sortRowsByDate(osakkeetCalculation.formData.sells), texts)
    );
  ...
  function createSubscriptionsSection(dataState, pageReadState, localizedTextNodes) {
    ...
    const rowsState = pageReadState.map(
      ({ osakkeetCalculation, texts }) => withSummaryRows(
      ...
      render: ({ row, removeButton }) => {
        ...
        const bindings = [
          ...
          },
          {
            ...
            setEditValue: (nextRow) => setInputValue(otherTotalAcquisitionCostsInput, nextRow.formRow.otherTotalAcquisitionCosts || "")
          }
        ];
        ...
        const syncSummaryCells = (nextRow) => {
          ...
          replaceChildren(
            ...
            nextRow.summary ? euro(
              nextRow.summary.shareCount.gt(0) ? nextRow.summary.baseShareAcquisitionCost.div(nextRow.summary.shareCount) : nextRow.summary.baseShareAcquisitionCost.mul(0)
            ) : "-"
          );
          replaceChildren(
            remainingCostPerShareCell,
            nextRow.summary ? euro(
              nextRow.summary.shareCount.gt(0) ? nextRow.summary.shareAcquisitionCost.div(nextRow.summary.shareCount) : nextRow.summary.shareAcquisitionCost.mul(0)
            ) : "-"
          );
        };
        const editableRow = createEditableRowManager(
        ...
        const renderHistoryTable = (historyRows, texts) => table(
        ...
        const syncHistoryVisibility = (nextRow) => {
          ...
          historyButton.textContent = isHistoryOpen ? nextRow.texts.subscriptions.history.hide : nextRow.texts.subscriptions.history.show;
  ...
  function createCashDistributionsSection(dataState, pageReadState, localizedTextNodes, commonTextNodes) {
    ...
    const openLotEventRowIds = /* @__PURE__ */ new Set();
    const lotEventRowSyncs = /* @__PURE__ */ new Map();
    ...
    const renderLotEventTable = (row) => {
      var _a2;
      const getDividendReasonText = (reason) => {
      ...
      const renderAllocationEffect = (allocation) => {
        var _a3, _b;
        const amountPerShare = ((_a3 = row.summary) == null ? void 0 : _a3.amountPerShare) || zero6;
        const appliedPerShare = allocation.shares.gt(0) ? allocation.capitalRepayment.div(allocation.shares) : zero6;
        const dividendPerShare = allocation.shares.gt(0) ? allocation.dividend.div(allocation.shares) : zero6;
        if (((_b = row.summary) == null ? void 0 : _b.type) === "dividend" || row.formRow.type === "dividend") {
      ...
      return table(
        ...
        tbody(
          ((_a2 = row.summary) == null ? void 0 : _a2.allocations.map(
            (allocation) => tr(
    ...
    const tbodyNode = createEditableCollectionTable({
      ...
      render: ({ row, removeButton }) => {
        ...
        const toggleLotEvents = (distributionId) => {
        ...
        const bindings = [
          ...
          },
          {
            ...
            readValue: (nextRow) => nextRow.formRow.type === "capital_return" ? nextRow.texts.cashDistributions.types.capitalReturn : nextRow.texts.cashDistributions.types.dividend,
        ...
        const syncDetailRow = (nextRow) => {
          const shouldShow = openLotEventRowIds.has(nextRow.id) && Boolean(nextRow.summary);
          lotEventButtonLabelNode.data = shouldShow ? nextRow.texts.cashDistributions.actions.hideLotEvents : nextRow.texts.cashDistributions.actions.showLotEvents;
  ...
  function createMathematicalShareValuesEditor(dataState, pageReadState, localizedTextNodes) {
    ...
    const rowsState = pageReadState.map(
      ({ osakkeetCalculation, texts }) => withRowActionLabels(osakkeetCalculation.formData.mathematicalShareValues, texts)
    );
  ...
  function createShareSplitsSection(dataState, pageReadState, localizedTextNodes, commonTextNodes) {
    ...
    const rowsState = pageReadState.map(
      ({ osakkeetCalculation, texts }) => withRowActionLabels(sortRowsByDate(osakkeetCalculation.formData.shareSplits), texts).map((row) => ({
        ...row,
        exampleEffectText: Number.isFinite(Number(row.multiplier.trim())) ? texts.shareSplits.fields.exampleEffectValue((100 * Number(row.multiplier.trim())).toFixed(2)) : "-"
      }))
    );
  ...
  function createDemergersSection(dataState, pageReadState, localizedTextNodes, commonTextNodes) {
    ...
    const rowsState = pageReadState.map(
      ({ osakkeetCalculation, texts }) => withRowActionLabels(sortRowsByDate(osakkeetCalculation.formData.demergers), texts).map((row) => {
        const ratioValue = Number(row.oldCompanyRatio.trim());
        return {
          ...row,
          exampleEffectText: Number.isFinite(ratioValue) ? texts.demergers.fields.exampleEffectValue(
  ...
  function buildRemainingShareBreakdown(osakkeetCalculation) {
    ...
    const zero7 = osakkeetCalculation.ipo.currentShareValue.mul(0);
    let totalShares = zero7;
    let vestedShares = zero7;
    let unvestedShares = zero7;
    let unvestedOriginalAcquisitionCost = zero7;
    for (const lot of osakkeetCalculation.subscriptions) {
      const remainingShares = decimal_default.max(lot.shareCount.minus(soldAmountByLotId.get(lot.id) || zero7), zero7);
  ...
  function renderDistributionSharesCell(row, styles2, t) {
    ...
    return div(
      ...
      p(styles2.compactParagraph, b(`${t.taxReturns.fields.distributionSharesDividend}: `), amount(row.dividendShareCount))
    );
  }
  ...
  function renderTaxTable(sectionSummary, showAllocationDetails, styles2, renderers, t) {
    ...
    const capitalRepaymentHeaderNode = mode === "unlisted" ? renderers.withHoverInfo(
        ...
        entries.flatMap((row) => [
          ...
          showAllocationDetails && row.type === "capital_return" && tr(
            td(
              ...
              styles2.historyCell,
              div(styles2.denseStack, b(t.taxReturns.sections.allocationDetails), renderAllocationTable(row, styles2, t))
            )
          )
        ]),
        tr(
  ...
  function renderTaxSectionWithToggle(title2, sectionSummary, styles2, renderers, t) {
    ...
    sync();
    return div(styles2.denseStack, h3(title2), div({ class: "no-print" }, styles2.rowButtons, toggleButton), contentRoot);
  }
  function renderSalesTable(sales, t) {
    if (sales.length === 0) return false;
    const saleRows = sales.flatMap(
      (sale) => sale.summary.usedLots.map((row) => ({
        ...row,
    ...
    return div(
      years.map(
        (yearSummary) => div(
          ...
          yearSummary.assets && div(styles2.denseStack, h3(t.taxReturns.sections.assets), renderAssetsTable(yearSummary.assets, t)),
          ...
          yearSummary.listed && renderTaxSectionWithToggle(t.taxReturns.sections.listed, yearSummary.listed, styles2, renderers, t),
          yearSummary.sales && yearSummary.sales.length > 0 && div(styles2.denseStack, h3(t.taxReturns.sections.ipoSale), renderSalesTable(yearSummary.sales, t))
        )
      )
    );
  }
  ...
  function createSellAllocationTable(osakkeetCalculation, texts, withHoverInfo2) {
    return table(
      ...
      tbody([
        osakkeetCalculation.ipoSell.usedLots.map(
          (lot) => tr(
            td(lot.lotDate || "-"),
  ...
  function createCashReserveCards(osakkeetCalculation, texts, infoCard2) {
    ...
    const ipoSharePrice = osakkeetCalculation.ipoSell.amount.gt(0) ? osakkeetCalculation.ipoSell.grossTotal.div(osakkeetCalculation.ipoSell.amount) : osakkeetCalculation.ipo.currentShareValue.mul(0);
    ...
    const keepAfterTaxesPercentage = osakkeetCalculation.ipoSell.grossTotal.gt(0) ? ` (${percentage(osakkeetCalculation.ipoSell.netAfterTaxAndSellCost.div(osakkeetCalculation.ipoSell.grossTotal).mul(100))})` : "";
    return [
      infoCard2(
  ...
  function createSaleResultComparisonCards(osakkeetCalculation, texts, infoCard2) {
    ...
    const netResultPercent = osakkeetCalculation.ipoSell.soldShareOriginalCostTotal.gt(0) ? percentage(
  ...
  function createAnnualAdjustmentCards(osakkeetCalculation, texts, infoCard2) {
    ...
    const annualKeepAfterTaxesPercentage = osakkeetCalculation.ipoSell.grossTotal.gt(0) ? ` (${percentage(osakkeetCalculation.ipoSell.netAfterAnnualTaxAndSellCost.div(osakkeetCalculation.ipoSell.grossTotal).mul(100))})` : "";
    return {
      taxEffect: infoCard2(
        texts.summary.ipoSell.cashReserve.taxEffectFromOtherAnnualCapital,
        osakkeetCalculation.ipoSell.taxReductionFromOtherLosses.gt(0) ? euro(osakkeetCalculation.ipoSell.taxReductionFromOtherLosses.mul(-1)) : osakkeetCalculation.ipoSell.annualTaxChange.gt(0) ? `+${euro(osakkeetCalculation.ipoSell.annualTaxChange)}` : euro(zeroMoney),
  ...
  function createResultsSection(dataState, pageReadState, localizedTextNodes) {
    ...
    const ipoSellInputHelpNodes = createComputedTextState(pageReadState, ({ osakkeetCalculation, texts }) => ({
      sharesToSellShareOfSellable: texts.summary.ipoSell.fields.sharesToSellShareOfSellable(
        osakkeetCalculation.vesting.vestedShares.gt(0) ? percentage(osakkeetCalculation.ipoSell.amount.div(osakkeetCalculation.vesting.vestedShares).mul(100)) : "0.00 %"
      )
    })).textNodes;
    const ipoSellOverviewRoot = div();
    const ipoSellDetailsRoot = div(pageStyles.denseStack);
    const annualAdjustmentSectionRoot = div();
    const annualAdjustmentCardsState = pageReadState.map(
      ({ osakkeetCalculation, texts }) => createAnnualAdjustmentCards(
        osakkeetCalculation,
        texts,
        (title2, value, help) => infoCard(pageStyles.summaryItem, pageStyles.cardMutedText, title2, value, help)
      )
    ...
    replaceChildrenFromState(pageReadState, ipoSellDetailsRoot, ({ osakkeetCalculation, texts }) => {
      ...
      return [
        ...
        hasUsableIpoSellCalculation && createSellAllocationTable(
          osakkeetCalculation,
          texts,
          (content, tooltip) => withHoverInfo(pageStyles.hoverInfo, pageStyles.hoverInfoIcon, content, tooltip)
        ),
        hasUsableIpoSellCalculation && h3(texts.summary.ipoSell.explanations.title),
        hasUsableIpoSellCalculation && div(
          pageStyles.summaryGrid,
  ...
  function createTopSection(dataState, pageReadState, languageSelectionState, localizedTextNodes, initialStatus = "") {
    ...
    const copyCurrentFullShareUrl = () => {
      void (async () => {
        try {
          const copied = await copyTextToClipboard(await buildFullShareUrl(dataState.get(), createId2));
          setStatus(copied ? currentTexts.storage.status.shareUrlCopied : currentTexts.storage.errors.clipboardFailed);
        } catch {
          setStatus(currentTexts.storage.errors.shareUrlUnavailable);
        }
  ...
  function createTaxSummarySection(pageReadState, localizedTextNodes) {
    ...
    replaceChildrenFromState(
      pageReadState,
      resultsRoot,
      ({ osakkeetCalculation, texts }) => createTaxSummaryContent(osakkeetCalculation, texts, pageStyles, {
        createActionButton: (labelNode, variant, onClick) => createActionButton(pageStyles.smallButton, labelNode, variant, onClick),
        infoCard: (title2, value, help) => infoCard(pageStyles.summaryItem, pageStyles.cardMutedText, title2, value, help),
  ...
  async function tryLoadFullSharedUrlData() {
